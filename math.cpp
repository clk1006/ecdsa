...
double* getZeros(double polyn[4]){
    ...
    if(delta>0){
        ...
        zeros[0]=-sqrt(-4/3*p)+cos(1/3*acos(-q/2*sqrt(-27/pow(p,3))))-B/(3*A);
        zeros[1]=-sqrt(-4/3*p)+cos(1/3*acos(-q/2*sqrt(-27/pow(p,3)))+M_PI/3)-B/(3*A);
        zeros[2]=-sqrt(-4/3*p)+cos(1/3*acos(-q/2*sqrt(-27/pow(p,3)))-M_PI/3)-B/(3*A);
    }
    return zeros;
}